...
\subsection{Neighbourhood-Distribution Distillation}

The estimator we minimise, a KL divergence between teacher and student softmax distributions over the similarities to a set of reference items, is not new as a loss.
It is the objective of stochastic neighbour embedding~\cite{hinton2002sne} with a cosine kernel, of relational knowledge distillation~\cite{park2019relational}, and, in the form closest to ours, of CompRess~\cite{koohpayegani2020compress} and SEED~\cite{fang2021seed}, which distil a self-supervised teacher into a smaller student by matching similarity distributions over a memory queue.
Matryoshka-Adaptor~\cite{yoon2024matryoshka} applies pairwise-similarity and top-$k$ neighbourhood preservation losses between truncated and full embeddings for the same purpose we have.
We therefore do not position the loss itself as the contribution.
What SDR-MRL adds is the rate-conditioned framing: the same estimator is applied to nested prefixes of a single encoder against a distribution over deployment rates, its value is given the information reading of Section~\ref{sec:method} together with the conditions under which that reading is exact, and the evaluation protocol of Section~\ref{sec:profile} measures the resulting successive refinement directly.
Whether this framing buys anything beyond the loss family it belongs to is precisely what ablation A5 and the comparison to Matryoshka-Adaptor are for.
...
This is too small to carry much semantics: with $B$ examples the variable $S$ has at most $\log(B-1)$ nats ($2.7$ nats at $B=16$), and for a corpus drawn from several unrelated tasks most anchors have no genuine neighbour in their batch, so $p_T$ is close to uniform and $\cD_k$ mostly measures the fine ordering of unrelated sentences.
We therefore augment $\mathcal C_i$ with a first-in-first-out memory queue $\mathcal Q$ of the full-dimensional embeddings of the last few thousand examples, in the manner of momentum-contrast and similarity-distribution distillation~\cite{koohpayegani2020compress,fang2021seed}.
Queue entries are candidates only: gradient flows through the anchor (Eq.~\eqref{eq:geometry-gradient}), and a queued row being a few steps stale is the usual approximation rather than a change of objective.
For the teacher side the queue stores the teacher's embedding of the same item, so that both distributions are defined over the same $\mathcal C_i$.
...
\paragraph{Decoder temperature.}
\label{sec:method-decoder-temperature}
The temperature $\tau_k$ is indexed by the prefix, and this is not cosmetic.
The optimal decoder of Section~\ref{sec:optimal} is the posterior $p_T(S|Z_k)$, an average of $p_T(S|X)$ over every input that shares the prefix $Z_k$; it is therefore \emph{flatter} than the teacher distribution, increasingly so as $d_k$ shrinks and more inputs collapse onto the same prefix.
A cosine decoder pinned to $\tau_k=\tau_T$ cannot express this: it is forced to be as confident as the teacher even where the prefix has lost the information that justified the confidence, and the excess is charged to the approximation gap $\epsilon_k$ of Proposition~\ref{prop:variational}.
Because $\cD_k^\star$ is an infimum over decoders, minimising over $\tau_k$ stays inside that infimum and can only tighten the bound.
We therefore treat $\{\tau_k\}_{k<K}$ as $K-1$ learnable decoder parameters (parametrised in log space, clamped to a fixed range), fix $\tau_K=\tau_T$ so that $\cD_K=0$ by construction, and discard them after training: they belong to the decoder, not to the representation.
Empirically the learned $\tau_k$ decreases monotonically toward $\tau_T$ as $d_k$ grows, which is the behaviour the posterior argument predicts.
...
\begin{remark}[What the proposition does and does not claim]
The decomposition is the standard variational identity behind the Barber--Agakov bound~\cite{barber2003algorithm} and behind every distillation objective with soft targets; we claim no novelty for it.
Its content here lies in two choices: the semantic variable $S$ is an \emph{inter-example} neighbourhood relation, and the decoder is the prefix's own cosine geometry, so the bound is on exactly the quantity that nearest-neighbour retrieval at rate $R_k$ depends on.
The identity also presupposes a \emph{fixed} joint $p_T(x,s)$.
When the teacher is the student's own full width (Section~\ref{sec:teacher}), $p_T$ moves with $\theta$ and $\cD_k$ becomes a self-consistency term rather than a bound on a fixed information quantity; Section~\ref{sec:task} discusses the consequence.
\end{remark}
...
To read Eq.~\eqref{eq:refinement-gain} as a statement about the \emph{added coordinates}, the new block must be defined so that $Z_k$ is a function of $(Z_{k-1},B_k)$ and vice versa.
For the raw prefixes this is immediate, $H_k=(H_{k-1},B_k)$.
For the normalised prefixes of Eq.~\eqref{eq:prefix} it is not: $Z_{k-1}$ discards the norm $\|h_{1:d_{k-1}}\|$, so $Z_k$ cannot be reconstructed from $Z_{k-1}$ and the raw block alone.
The correct object is the \emph{normalised block}
\begin{equation}
    \widetilde B_k
    =
    \left(
        \rho_k,\;
        \frac{h_{d_{k-1}+1:d_k}}{\|h_{1:d_k}\|}
    \right),
    \qquad
    \rho_k=\frac{\|h_{1:d_{k-1}}\|}{\|h_{1:d_k}\|},
\end{equation}
for which $Z_k=(\rho_k Z_{k-1},\,h_{d_{k-1}+1:d_k}/\|h_{1:d_k}\|)$ is a bijective function of $(Z_{k-1},\widetilde B_k)$, and therefore
\begin{equation}
\boxed{
    \cD_{k-1}^\star-\cD_k^\star
    =
    I_T(S;\widetilde B_k|Z_{k-1}).
}
\label{eq:block-information}
\end{equation}
The extra scalar $\rho_k$ carries how much of the prefix norm the new coordinates contribute; in the raw parametrisation it is absorbed into $B_k$ and Eq.~\eqref{eq:block-information} reduces to $I_T(S;B_k|Z_{k-1})$.
...
\paragraph{What the task objective does not prevent.}
Collapse of the whole representation is the crude failure mode; there is a subtler one that the task loss does not exclude.
With an online self-teacher, $\cD_k=0$ is reached whenever the prefix's cosine geometry coincides with the full width's, and this is equally achieved by the prefix acquiring the tail's information or by the tail blocks $B_{k+1},\ldots,B_K$ becoming semantically \emph{inert} (near-constant, or contributing no inter-example structure).
The stop-gradient of Eq.~\eqref{eq:teacher} removes the direct gradient path into the teacher but not the shared parameters, and an in-batch contrastive task loss with small $B$ saturates long before it constrains the tail.
Consequently the set of global minimisers of $\cL_{\mathrm{task}}+\lambda_{\mathrm{sem}}\cL_{\mathrm{sem}}$ contains tail-inert solutions, in which the ``lost semantic information'' $I_T(S;X|Z_k)$ is small because $I_T(S;X)$ itself has shrunk.
Three consequences follow for the method.
(i) The information reading of Proposition~\ref{prop:variational} is exact only for a teacher whose $p_T$ does not depend on the current student, i.e. the EMA or frozen teacher of Section~\ref{sec:teacher}; the online variant should be understood as a self-consistency regulariser, and the teacher ablation (A3) is part of the method rather than an afterthought.
(ii) Evaluation must use a reference teacher held fixed across models (Section~\ref{sec:profile}), never the model's own full width.
(iii) Training diagnostics track the \emph{norm share} $\E\,\|h_{1:d_k}\|^2/\|h\|^2$ of each prefix; the smallest prefix's share climbing toward one is the signature of a tail being emptied rather than a prefix being improved.
...
The stop-gradient on $\cD_{k-1}$ is essential.
Without it, $\partial[\cD_k-\cD_{k-1}]_+/\partial\cD_{k-1}=-1$ on every violated edge, so the cheapest way to satisfy the constraint is to make the \emph{smaller} prefix worse, which is the opposite of what a Matryoshka objective wants.
With the stop-gradient the lower prefix acts as a target and the hinge can only pull $\cD_k$ down.
...
For monotonic regularization we reuse the sampled $K_t$ and compute only the two prefix distortions of the edge $(K_t-1,K_t)$.
Since that edge is drawn with probability $\pi_{K_t}$ while Eq.~\eqref{eq:mono} is an unweighted sum, the edge term is importance-weighted:
\begin{equation}
    \widehat{\cL}_{\mathrm{mono}}
    =
    \frac{1}{\pi_{K_t}}
    \left[
        \cD_{K_t}-\sg(\cD_{K_t-1})
    \right]_+,
    \qquad
    \E_{K_t\sim\pi}\,\widehat{\cL}_{\mathrm{mono}}
    =
    \cL_{\mathrm{mono}}.
\end{equation}
Under a strongly low-rate-focused prior the weight $1/\pi_{K_t}$ is large for the widest prefixes, so the variance of this estimator grows with the skew of $\pi$; the full-prefix computation should be preferred whenever $\lambda_{\mathrm{mono}}>0$ and $\pi$ is far from uniform.
...
\Require batch $\mathcal B$, memory queue $\mathcal Q$, encoder $f_\theta$, dimensions $\{d_k\}$,
rate distribution $\pi$, teacher temperature $\tau_T$, decoder temperatures $\{\tau_k\}_{k<K}$
...
\operatorname{Softmax}
(
Z_k [Z_k;\mathcal Q_{1:d_k}]^\top/\tau_k
)$ \Comment{candidates: batch $\cup$ queue, self excluded}
...
\State update $\theta$ and $\{\tau_k\}$ using $\nabla\cL$
\State push $\sg(Z_K)$ (and the teacher's $\bar Z_K$) into $\mathcal Q$, FIFO
...
It is, however, the only one of the three for which Proposition~\ref{prop:variational} is not literally a statement about a fixed semantic variable (Section~\ref{sec:task}); the EMA teacher is the cheapest variant that restores that reading, and a large online--EMA gap in the ablation should be read as the tail-inert degeneracy being real rather than as ``teacher drift'' in the abstract.
...
For the same reason as in Section~\ref{sec:method-decoder-temperature}, $\cD_k^{\mathrm{ref}}$ is reported at the best decoder temperature for that prefix,
\begin{equation}
    \cD_k^{\mathrm{ref}}
    =
    ...
    \right),
    \qquad
    \mathcal T=\{\tau_T\cdot c : c\in\{1,1.25,\ldots,10\}\},
\end{equation}
which is the infimum over the cosine-decoder family and hence the tightest value of the Proposition~\ref{prop:variational} bound the protocol can state.
Without this a model would be charged for a prefix that is merely less \emph{certain} than the reference rather than wrong about the neighbourhood ordering, and the charge would fall hardest on exactly the low rates the protocol is meant to compare.
The selected temperatures are reported alongside the profile.
...
\{0,0.05,0.1,0.3\}.
\end{align}
The range for $\lambda_{\mathrm{sem}}$ reflects the relative scale of the two terms: $\cL_{\mathrm{sem}}$ is a $\pi$-weighted \emph{average} over $K-1$ prefixes while $\cL_{\mathrm{task}}$ is a \emph{sum} over $K$ InfoNCE terms of the same temperature, so at $\lambda_{\mathrm{sem}}=1$ the semantic gradient is roughly $1/K$ of the task gradient at initialisation.

The candidate set contains all in-batch examples plus the memory queue of Section~\ref{sec:method}, with $|\mathcal Q|=4096$ initially.
...
\bibitem{koohpayegani2020compress}
